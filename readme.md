...
可以在`./.github/workflows/zdm_crawler.yml`的`main logic.env`中配置全局价格区间：

* `minPrice`：最低价格，包含该价格；留空表示不限制
* `maxPrice`：最高价格，包含该价格；留空表示不限制
* 未配置任何价格条件时，保持原有行为，不会因为价格格式无法识别而过滤商品

如果需要给不同商品设置不同价格区间，在仓库根目录的`price_rules.txt`中每行配置一条规则，格式如下：
...
规则按标题关键词匹配，价格边界包含在区间内。**规则文件非空时按白名单模式运行：只推送命中规则且价格在区间内的商品；未命中任何规则的商品不推送。** 商品命中单品规则时，单品规则优先于全局价格区间；规则文件为空时使用全局区间，全局区间也为空则不过滤价格。价格支持整数、小数、千位逗号以及接口常见的`¥`/`元`文本。规则文件路径可以通过`PRICE_RULES_FILE`环境变量调整。
